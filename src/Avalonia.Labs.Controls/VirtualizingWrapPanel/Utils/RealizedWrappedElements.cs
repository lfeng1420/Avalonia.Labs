...
namespace Avalonia.Labs.Controls.Utils
{
    ...
    internal class RealizedWrappedElements
    {
        ...
        {
            var length = index * estimate.U;
            double averageULength = 0;
            while (averageULength < length)
            {
                var newU = averageULength + estimate.U;
                if (newU > viewportEnd.U)
                    break;
            ...
            return new UVSize(viewportEnd.Orientation)
            {
                ...
                V = (int)(length / averageULength) * estimate.V
            };
        }
        ...
        {
            var total = new UVSize(orientation);
            var divisor = 0.0;
    ...
    internal struct UVSize
    {
        ...
        public static UVSize operator +(UVSize left, UVSize right)
        {
            if (left.Orientation != right.Orientation)
            {
                throw new InvalidOperationException("Adding UVSize of different orientations is not supported");
            }

            return new UVSize(left.Orientation)
            {
                U = left.U + right.U,
                V = Math.Max(left.V, right.V)
            };
        }

        public static UVSize operator /(UVSize left, double right)
        {
            return new UVSize(left.Orientation)
            {
                U = left.U / right,
        ...
        public static bool operator ==(UVSize left, UVSize right)
        {
            if (left.Orientation != right.Orientation)
            {
                throw new InvalidOperationException("Comparing UVSize of different orientations is not supported");
            }
