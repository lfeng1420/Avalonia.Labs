...
namespace Avalonia.Labs.Controls
{
    ...
    public class VirtualizingWrapPanel : VirtualizingPanel
    {
        ...
        protected override Control? ScrollIntoView(int index)
        {
            ...
            else if (this.GetVisualRoot() is ILayoutRoot root)
            {
                ...
                var rect = Orientation == Orientation.Horizontal ?
                    new Rect(anchorUV.U, anchorUV.V, _scrollToElement.DesiredSize.Width, _scrollToElement.DesiredSize.Height) :
                    new Rect(anchorUV.V, anchorUV.U, _scrollToElement.DesiredSize.Width, _scrollToElement.DesiredSize.Height);
        ...
        {
            if (_realizedElements is null)
                return _lastEstimatedElementSizeUV;

            var result = _realizedElements.EstimateElementSize(Orientation);
            if (result != null)
                _lastEstimatedElementSizeUV = result.Value;
            return _lastEstimatedElementSizeUV;
        }
